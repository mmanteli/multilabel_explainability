...
def write_kw(df, multilabels = None):
    ...
    if multilabels == None:
        for i in set(df['pred_label']):
            filename = 'kw_results/kw_label_'+str(i)+'.txt'
            f = open(filename, 'w')
            f.write("Class "+str(i)+" ("+labels[int(i)]+")\n")
            kw = df[(df.freq >= 2) & (df.pred_label == i) & (df.class_freq < 4)]
...
data = pd.read_csv('test_en.tsv', sep = '\t', names = ['document_id', 'pred_label', 'token', 'score'])
